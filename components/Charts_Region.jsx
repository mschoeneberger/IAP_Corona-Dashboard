    ...
    useEffect(()=>{
            ...
            if(props.activeRegion){
                ...
                for(var selected of props.selectedRegions){
                    if(selected.value === props.activeRegion.country && selected.label === props.activeRegion.region) {
                        notselected = false
                        break
                    }
                }
                if(notselected) {
                    ...
                    if(showing.length >= number_of_colors)   return props.alert.show("Maximum number of countries reached")
                        ...
                        var obj = {}
                        obj.value = props.activeRegion.country.toLowerCase()
                        obj.label = props.activeRegion.region
                        props.setSelectedRegions(x => [...x,obj])
                    }                                     
                }
            }
    ...
    useEffect(() => {
        ...
        if(chart){
            ...
            for(var selected of compare_selected){
                ...
                if(props.completeRegionData[0][selected.value].length === 1){
