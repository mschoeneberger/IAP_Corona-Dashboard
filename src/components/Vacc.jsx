    ...
    useEffect(() => {
        if(props.completeRegionData){
            if(props.completeRegionData.length > 1){
                ...
                // Set German Population
                German_population = 80000000
                /*
    ...
    useEffect(() => {
        if(props.completeRegionData){
            if(props.completeRegionData.length > 1){
                ...
                var [data,labels] = getRelevantData(VaccData,"first vacc", props.startDate, props.endDate, step, new Date(props.lastUpdate),relativeState)
                var color = "rgb(75, 192, 192)"
                chart.data.datasets.push({
                    label: labels.dataset[props.activeLanguage][0],
                    data: data,
                    //color
                ...
                //Second Vacc
                var [data,labels] = getRelevantData(VaccData,"second vacc", props.startDate, props.endDate, step, new Date(props.lastUpdate),relativeState)
                var color = "rgb(75, 192, 150)"
                chart.data.datasets.push({
                    label: labels.dataset[props.activeLanguage][1],
                    data: data,
                    //color
                    backgroundColor: chart_color(color).alpha(0.2).rgbString(),
                    borderColor: color,
                ...
                // Chart Labels
                chart.options.title.text = labels.title[props.activeLanguage]
