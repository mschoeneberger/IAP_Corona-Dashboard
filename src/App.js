...
import play from './tasks/playTask.js'
...
const App = () => {
    ...
    const load = () => {
        ...
        loadCountriesTask.load(setCountries, setLastUpdate, setCompleteData, setAlldates);
        const loadEuropeTask = new LoadEuropeTask();
        loadEuropeTask.load(setEuropeCountries, setCompleteRegionData);
    };

    useEffect(load, []);
    useEffect(() => {play(playStatus, mapDate, setMapDate, setPlayStatus)},[playStatus,mapDate]);  
        ...
        <div style={{overflow: "hidden"}}>
            <div className="page">
                ...
                <div style={{height:"90%", width:"100%", display:"flex", flexDirection:"row"}}>
                    ...
                    <div style={{height:"100%", flexBasis:"75vh", flexGrow:"2", display:"flex", flexDirection:"column"}}>
                        {/* This div is only there to fix a visual glitch when changing focus */}
                        <div style={{flexGrow:"1", flexBasis:"75vh", width:"100%", display:"flex"}}>
                            ...
                            {activeFocus === "World" ? (countries.length === 0 ? (
                               ...
                               <LoadingMap/>
                                    ...
                                    <CovidMap countries={countries} legends={legends} active={activeLegend} activeLanguage={activeLanguage} setActiveCountry={setActiveCountry} mapDate={mapDate} alldates={alldates}/>
                                </>
                                  
                            )) : (europeCountries.length === 0 ? (
                               <LoadingMap/>
                            ) : (<>
                                    <EuropeCovidMap rerenderMap={completeRegionData} regions={europeCountries} legends={regionLegends} active={activeLegend} activeLanguage={activeLanguage} setActiveRegion={setActiveRegion} mapDate={mapDate} alldates={alldates}/>
                                </>
                            ))}
                        </div>
                        <Timerow playStatus={playStatus} setPlayStatus={setPlayStatus} alldates={alldates} mapDate={mapDate} setMapDate={setMapDate} activeLanguage={activeLanguage} focus={activeFocus} countries={countries} regions={europeCountries}/>
